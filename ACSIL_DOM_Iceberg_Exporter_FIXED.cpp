...
namespace
{
    ...
    const double DOM_EXPORTER_MILLISECONDS_PER_DAY = 86400000.0;
    ...
    void FlushIfNeeded(ExporterState& State, const int FlushEveryRows)
    {
        const unsigned long long EffectiveFlushInterval =
            static_cast<unsigned long long>(FlushEveryRows > 0 ? FlushEveryRows : 1);
    ...
    {
        const int ReportedBidLevels = Sc.GetBidMarketDepthNumberOfLevels();
        const int ReportedAskLevels = Sc.GetAskMarketDepthNumberOfLevels();
        const int AvailableBidLevels = ReportedBidLevels > 0 ? ReportedBidLevels : 0;
        const int AvailableAskLevels = ReportedAskLevels > 0 ? ReportedAskLevels : 0;
        const int EffectiveRequestedLevels = RequestedLevels > 0 ? RequestedLevels : 1;

        const int BidLevels = AvailableBidLevels < EffectiveRequestedLevels
            ? AvailableBidLevels
            : EffectiveRequestedLevels;
        const int AskLevels = AvailableAskLevels < EffectiveRequestedLevels
            ? AvailableAskLevels
            : EffectiveRequestedLevels;
...
SCSFExport scsf_DOMIcebergCSVExporter(SCStudyInterfaceRef sc)
{
    ...
    if (!SnapshotIsDue)
    {
        const double ElapsedMilliseconds =
            ...
            * DOM_EXPORTER_MILLISECONDS_PER_DAY;
