...
/*
    ...
    Revision: 1.1.0 - high-confidence BUY/SELL output
...
#pragma pack(pop)
    ...
    struct CandleSignalState
    {
        int Direction = DIRECTION_NONE;
        int Score = -1;
        int Priority = 0;
        int SourceType = SIGNAL_NONE;
        double EventDateTime = 0.0;
        bool Notified = false;
    };
    ...
    struct StudyState
    {
        ...
        std::map<int, int> SignalLabelLineNumbers;
        std::map<int, CandleSignalState> CandleSignals;
    };
    ...
    void DeleteStudyDrawings(SCStudyInterfaceRef sc, StudyState& State)
    {
        ...
        for (const auto& Pair : State.SignalLabelLineNumbers)
        {
            if (Pair.second != 0)
                sc.DeleteACSChartDrawing(sc.ChartNumber, TOOL_DELETE_CHARTDRAWING, Pair.second);
        }
        State.SignalLabelLineNumbers.clear();
    }
    ...
    int SignalDirection(const int Type)
    {
        switch (Type)
        {
            case SIGNAL_BID_ABSORPTION:
            case SIGNAL_SELLER_EXHAUSTION:
            case SIGNAL_ASK_ABSORBER_EXHAUSTED:
                return DIRECTION_BUY;

            case SIGNAL_ASK_ABSORPTION:
            case SIGNAL_BUYER_EXHAUSTION:
            case SIGNAL_BID_ABSORBER_EXHAUSTED:
    ...
    bool IsConfirmedDirectionalSignal(const int Type)
    {
        return Type == SIGNAL_SELLER_EXHAUSTION
            || Type == SIGNAL_BUYER_EXHAUSTION
            || Type == SIGNAL_BID_ABSORBER_EXHAUSTED
            || Type == SIGNAL_ASK_ABSORBER_EXHAUSTED;
    }

    bool IsStrongerSignal(const SignalEvent& Candidate, const SignalEvent* Existing)
    {
        if (Existing == nullptr)
            return true;
        if (Candidate.Score != Existing->Score)
            return Candidate.Score > Existing->Score;
        return SignalPriority(Candidate.Type) > SignalPriority(Existing->Type);
    }

    void DeleteSignalLabelForBar(
        SCStudyInterfaceRef sc,
        StudyState& State,
        const int BarIndex)
    {
        const auto Existing = State.SignalLabelLineNumbers.find(BarIndex);
        if (Existing == State.SignalLabelLineNumbers.end())
            return;

        if (Existing->second != 0)
            sc.DeleteACSChartDrawing(sc.ChartNumber, TOOL_DELETE_CHARTDRAWING, Existing->second);
        State.SignalLabelLineNumbers.erase(Existing);
    }

    void PlotBuySellSignal(
        SCStudyInterfaceRef sc,
        StudyState& State,
        const int BarIndex,
        const CandleSignalState& Signal,
        const int MarkerOffsetTicks,
        const int LabelOffsetTicks,
        const bool DrawLabels,
        const int LabelFontSize,
        const COLORREF BuyColor,
        const COLORREF SellColor)
    {
        if (BarIndex < 0 || BarIndex >= sc.ArraySize || sc.TickSize <= 0.0f)
            return;
        ...
        {
            DeleteSignalLabelForBar(sc, State, BarIndex);
            return;
        }

        const bool IsBuy = Signal.Direction == DIRECTION_BUY;
        const double CandleAnchor = IsBuy ? sc.Low[BarIndex] : sc.High[BarIndex];
        const double MarkerPrice = CandleAnchor
            + (IsBuy ? -MarkerOffsetTicks : MarkerOffsetTicks) * sc.TickSize;
        sc.Subgraph[IsBuy ? 0 : 1][BarIndex] = static_cast<float>(MarkerPrice);
        ...
        const int EffectiveLabelOffsetTicks = std::max(
            LabelOffsetTicks,
            MarkerOffsetTicks + 1);
        ...
        Tool.BeginValue = CandleAnchor
            + (IsBuy ? -EffectiveLabelOffsetTicks : EffectiveLabelOffsetTicks) * sc.TickSize;
        Tool.Color = IsBuy ? BuyColor : SellColor;
        ...
        if (sc.UseTool(Tool) != 0)
            State.SignalLabelLineNumbers[BarIndex] = Tool.LineNumber;
    }
...
SCSFExport scsf_MBOAbsorptionExhaustionSignals(SCStudyInterfaceRef sc)
{
    ...
    SCSubgraphRef UnusedSignal2 = sc.Subgraph[2];
    SCSubgraphRef UnusedSignal3 = sc.Subgraph[3];
    SCSubgraphRef UnusedSignal4 = sc.Subgraph[4];
    SCSubgraphRef UnusedSignal5 = sc.Subgraph[5];
    ...
    SCInputRef MinimumSignalConfidenceScore = sc.Input[63];
    SCInputRef MinimumOpposingScoreAdvantage = sc.Input[64];

    if (sc.SetDefaults)
    {
        sc.GraphName = "MBO Absorption & Exhaustion - High Confidence BUY / SELL";
        sc.StudyDescription = "Internally evaluates MBO absorption and exhaustion, but publishes only one high-confidence BUY or SELL signal per candle.";
        sc.AutoLoop = 0;
        sc.GraphRegion = 0;
        sc.UpdateAlways = 1;
        ...
        BuySignal.PrimaryColor = RGB(0, 230, 90);
        BuySignal.LineWidth = 4;
        BuySignal.DrawZeros = false;

        SellSignal.Name = "SELL";
        SellSignal.DrawStyle = DRAWSTYLE_ARROW_DOWN;
        SellSignal.PrimaryColor = RGB(245, 70, 70);
        SellSignal.LineWidth = 4;
        SellSignal.DrawZeros = false;

        UnusedSignal2.Name = "";
        UnusedSignal2.DrawStyle = DRAWSTYLE_IGNORE;
        UnusedSignal2.DrawZeros = false;
        UnusedSignal3.Name = "";
        UnusedSignal3.DrawStyle = DRAWSTYLE_IGNORE;
        UnusedSignal3.DrawZeros = false;
        UnusedSignal4.Name = "";
        UnusedSignal4.DrawStyle = DRAWSTYLE_IGNORE;
        UnusedSignal4.DrawZeros = false;
        UnusedSignal5.Name = "";
        UnusedSignal5.DrawStyle = DRAWSTYLE_IGNORE;
        UnusedSignal5.DrawZeros = false;
        ...
        DrawActiveLevelLines.Name = "Draw Active Absorption Level Lines (optional)";
        DrawActiveLevelLines.SetYesNo(0);

        DisplayLinePrice.Name = "Display Price On Active Level Lines";
        DisplayLinePrice.SetYesNo(0);

        ActiveLineWidth.Name = "Active Level Base Line Width";
        ActiveLineWidth.SetInt(2);
        ActiveLineWidth.SetIntLimits(1, 10);

        DrawSignalLabels.Name = "Draw BUY / SELL Text";
        DrawSignalLabels.SetYesNo(1);

        MarkerOffsetTicks.Name = "BUY / SELL Marker Offset From Candle (ticks)";
        MarkerOffsetTicks.SetInt(2);
        MarkerOffsetTicks.SetIntLimits(0, 20);

        LabelOffsetTicks.Name = "BUY / SELL Text Offset From Candle (ticks)";
        LabelOffsetTicks.SetInt(4);
        ...
        BullishExhaustionColor.Name = "BUY Signal Color";
        BullishExhaustionColor.SetColor(RGB(0, 230, 90));

        BearishExhaustionColor.Name = "SELL Signal Color";
        ...
        WriteSignalsToLog.Name = "Write BUY / SELL To Message Log";
        WriteSignalsToLog.SetYesNo(0);

        MinimumSignalConfidenceScore.Name = "Minimum Confirmed BUY / SELL Confidence Score (0-100)";
        MinimumSignalConfidenceScore.SetInt(80);
        MinimumSignalConfidenceScore.SetIntLimits(1, 100);

        MinimumOpposingScoreAdvantage.Name = "Minimum Score Advantage If BUY And SELL Conflict On Same Candle";
        MinimumOpposingScoreAdvantage.SetInt(5);
        MinimumOpposingScoreAdvantage.SetIntLimits(0, 50);
        return;
    }
    ...
    BuySignal.PrimaryColor = BullishExhaustionColor.GetColor();
    SellSignal.PrimaryColor = BearishExhaustionColor.GetColor();
    ...
    struct BarSignalCandidates
    {
        const SignalEvent* Buy = nullptr;
        const SignalEvent* Sell = nullptr;
    };
    ...
    const int MinimumConfidence = Clamp(MinimumSignalConfidenceScore.GetInt(), 1, 100);
    const int OpposingAdvantage = Clamp(MinimumOpposingScoreAdvantage.GetInt(), 0, 50);
    std::map<int, BarSignalCandidates> CandidatesByBar;

    for (const SignalEvent& Event : Signals)
    {
        // Raw absorption remains an internal prerequisite. Only confirmed
        // aggressor exhaustion or absorber failure can publish BUY / SELL.
        if (!IsConfirmedDirectionalSignal(Event.Type)
            || Event.Score < MinimumConfidence)
        {
            continue;
        }

        const int Direction = SignalDirection(Event.Type);
        const int BarIndex = BarIndexForDateTime(sc, Event.DateTime);
        if (Direction == DIRECTION_NONE || BarIndex < 0)
            continue;

        BarSignalCandidates& Candidates = CandidatesByBar[BarIndex];
        const SignalEvent*& Slot = Direction == DIRECTION_BUY
            ? Candidates.Buy
            : Candidates.Sell;
        if (IsStrongerSignal(Event, Slot))
            Slot = &Event;
    }

    std::vector<PublishedSignal> PublishedSignals;
    for (const auto& Pair : CandidatesByBar)
    {
        const int BarIndex = Pair.first;
        const BarSignalCandidates& Candidates = Pair.second;
        const SignalEvent* Selected = nullptr;

        if (Candidates.Buy != nullptr && Candidates.Sell != nullptr)
        {
            const int ScoreDifference = std::abs(Candidates.Buy->Score - Candidates.Sell->Score);
            if (ScoreDifference < OpposingAdvantage)
                continue;

            if (Candidates.Buy->Score > Candidates.Sell->Score)
                Selected = Candidates.Buy;
            else if (Candidates.Sell->Score > Candidates.Buy->Score)
                Selected = Candidates.Sell;
            else if (SignalPriority(Candidates.Buy->Type) > SignalPriority(Candidates.Sell->Type))
                Selected = Candidates.Buy;
            else if (SignalPriority(Candidates.Sell->Type) > SignalPriority(Candidates.Buy->Type))
                Selected = Candidates.Sell;
            else
                continue;
        }
        ...
        {
            Selected = Candidates.Buy != nullptr ? Candidates.Buy : Candidates.Sell;
        }

        if (Selected == nullptr)
            continue;

        const int Direction = SignalDirection(Selected->Type);
        CandleSignalState& Candle = State->CandleSignals[BarIndex];
        const bool HadPublishedSignal = Candle.Direction != DIRECTION_NONE;
        bool Accept = !HadPublishedSignal;

        if (HadPublishedSignal && Candle.Direction == Direction)
        {
            Accept = Selected->Score > Candle.Score
                || (Selected->Score == Candle.Score
                    && SignalPriority(Selected->Type) > Candle.Priority);
        }
        else if (HadPublishedSignal && Candle.Direction != Direction)
        {
            Accept = Selected->Score >= Candle.Score + OpposingAdvantage;
        }

        if (!Accept)
            continue;

        const bool WasNotified = Candle.Notified;
        Candle.Direction = Direction;
        Candle.Score = Selected->Score;
        Candle.Priority = SignalPriority(Selected->Type);
        Candle.SourceType = Selected->Type;
        Candle.EventDateTime = Selected->DateTime;
        Candle.Notified = WasNotified;
        ...
        if (!Candle.Notified)
        {
            PublishedSignals.push_back(PublishedSignal{BarIndex, Direction, Candle.Score});
            Candle.Notified = true;
        }
    }

    // Keep a signal on the forming candle outside the candle even if its high
    // or low expands after the signal was first published.
    if (sc.ArraySize > 0)
    {
        const int FirstRecentBar = std::max(0, sc.ArraySize - 2);
        for (const auto& Pair : State->CandleSignals)
        {
            if (Pair.first < FirstRecentBar || Pair.first >= sc.ArraySize)
                continue;
    ...
    const PublishedSignal* AlertSignal = nullptr;
    for (const PublishedSignal& Published : PublishedSignals)
    {
        SCString Message;
        Message = Published.Direction == DIRECTION_BUY ? "BUY" : "SELL";

        if (WriteSignalsToLog.GetYesNo() != 0 && !sc.IsFullRecalculation)
            sc.AddMessageToLog(Message, 0);

        if (AlertSignal == nullptr
            || Published.BarIndex > AlertSignal->BarIndex
            || (Published.BarIndex == AlertSignal->BarIndex && Published.Score > AlertSignal->Score))
        {
            AlertSignal = &Published;
        }
    }

    if (EnableAlerts.GetYesNo() != 0
        && AlertSignal != nullptr
        && !sc.IsFullRecalculation)
    {
        SCString AlertMessage;
        AlertMessage = AlertSignal->Direction == DIRECTION_BUY ? "BUY" : "SELL";
        sc.SetAlert(
            Clamp(AlertNumber.GetInt(), 0, 150),
            AlertSignal->BarIndex,
            AlertMessage);
    }
