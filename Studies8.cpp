...
SCSFExport scsf_StudyOverlayOHLC(SCStudyInterfaceRef sc)
{
	...
	c_ArrayWrapper<float>* p_ReferenceOpen = nullptr;
	c_ArrayWrapper<float>* p_ReferenceHigh = nullptr;
	c_ArrayWrapper<float>* p_ReferenceLow = nullptr;
	c_ArrayWrapper<float>* p_ReferenceClose = nullptr;
...
SCSFExport scsf_VolumeAtPriceArrayTest(SCStudyInterfaceRef sc)
{
	...
	for (int VAPIndex = 0; VAPIndex < VAPSizeAtBarIndex; VAPIndex++)
	{
		...
		TotalVolume += p_VolumeAtPrice->GetVolume();
		...
		double AskVolume = p_VolumeAtPrice->GetAskVolume();
		double BidVolume = p_VolumeAtPrice->GetBidVolume();
	...
	for (int ElementIndex = 0;ElementIndex < Count; ElementIndex ++)
	{
			...
			if (p_VolumeAtPriceAtIndex)
				TotalVolume += p_VolumeAtPriceAtIndex->GetVolume();
...
SCSFExport scsf_StochasticMomentumIndicator(SCStudyInterfaceRef sc)
{
	...
	{
		Subgraph_SMI[sc.Index] = Subgraph_SMI[sc.Index - 1];
	}
...
SCSFExport scsf_TimeRangeHighlight(SCStudyInterfaceRef sc)
{
	...
	SCInputRef Input_Version = sc.Input[2];
	SCInputRef Input_UseChartStandardSessionTimes = sc.Input[3];
	...
	if (sc.SetDefaults)
	{
		...
		Input_UseChartStandardSessionTimes.Name = "Use Chart Standard Session Times";
		Input_UseChartStandardSessionTimes.SetYesNo(false);
	...
	if (sc.IsFullRecalculation && Input_UseChartStandardSessionTimes.GetYesNo())
	{
		Input_StartTime.SetTime(sc.StartTime1);
		Input_EndTime.SetTime(sc.EndTime1);
	}
...
SCSFExport scsf_AskBidVolumeDifferenceBars(SCStudyInterfaceRef sc)
{
		...
		&& static_cast<int>(sc.PullbackVolumeAtPrice->GetNumberOfBars()) > 0)
	{
		...
		for (int VAPIndex = 0; VAPIndex < VAPSizeAtBarIndex; VAPIndex++)
		{
			...
			PullBackAskVolumeBidVolumeDifference += p_VolumeAtPrice->GetAskVolume() - p_VolumeAtPrice->GetBidVolume();
...
SCSFExport scsf_VolumeAtPriceThresholdAlert(SCStudyInterfaceRef sc)
{
	...
	for (int Index = sc.UpdateStartIndex; Index < sc.ArraySize; Index++)
	{
		...
		for (int VAPIndex = 0; VAPIndex < NumVAPElementsAtBarIndex; VAPIndex++)
		{
			...
			{
				Volume = p_VolumeAtPrice->GetBidVolume();
				...
				if (Volume >= VolumeThreshold)
				{
					...
					{
						AlertMessage.Format("Bid Volume %f triggered alert at %s", 
							Volume, 
			...
			{
				Volume = p_VolumeAtPrice->GetAskVolume();
				...
				if (Volume >= VolumeThreshold)
				{
					...
					{
						AlertMessage.Format("Ask Volume %f triggered alert at %s", 
							Volume, 
							sc.FormatGraphValue(Price, sc.ValueFormat).GetChars()
							);
			...
			{
				Volume = p_VolumeAtPrice->GetVolume();
				...
				if (Volume >= VolumeThreshold)
				{
					...
					{
						AlertMessage.Format("Total Volume %f triggered alert at %s", 
							Volume,
							sc.FormatGraphValue(Price, sc.ValueFormat).GetChars()
							);
...
SCSFExport scsf_RelativeVolume(SCStudyInterfaceRef sc)
{
	...
	SCSubgraphRef Subgraph_EarliestReferenceDate = sc.Subgraph[8];
	...
	SCInputRef Input_EnableDebugLogging = sc.Input[12];
...
// 	SCInputRef Input_DisplayCumulativeVolumeAverageCumulativeVolumeDiff = sc.Input[15];
	...
	if (sc.SetDefaults)
	{
		...
		Subgraph_VolumeAverageVolumeDiff.Name = "Volume - Average Volume Diff";
		...
		Subgraph_CumulativeVolumeAverageCumulativeVolumeDiff.Name = "Cumulative Vol Average - Cumulative Vol";
		...
		Subgraph_EarliestReferenceDate.Name = "EarliestReferenceDate";
		Subgraph_EarliestReferenceDate.DrawStyle = DRAWSTYLE_IGNORE;
		Subgraph_EarliestReferenceDate.LineWidth = 1;
		Subgraph_EarliestReferenceDate.PrimaryColor = RGB(0, 255, 0);
		Subgraph_EarliestReferenceDate.ValueFormat = VALUEFORMAT_DATE;

		Input_Period.Name = "Period - Number of Days to Include";
		...
		Input_EnableDebugLogging.Name = "Enable Debug Logging";
		Input_EnableDebugLogging.SetYesNo(false);
...
// 		Input_DisplayCumulativeVolumeAverageCumulativeVolumeDiff.SetYesNo(false);
	...
	int& r_LastIndex = sc.GetPersistentInt(1);
...
//		}
	...
	for (int BarIndex = sc.UpdateStartIndex; BarIndex < sc.ArraySize; BarIndex++)
	{
		...
		const bool IsNewDay = BarIndex == 0 || CurrentBarTradeDate != PriorBarTradeDate;

		Array_TimespanForDay[BarIndex] = static_cast<float>(sc.TimeSpanOfBar(BarIndex).GetAsDouble());

		if (BarIndex >= 1 && !IsNewDay)
		{
			// For the time span 1.0 equals 1 Day. Therefore, this can be compared to a fraction where 1.0 is 100% and where .01 is 1%.
			Array_TimespanForDay[BarIndex] += Array_TimespanForDay[BarIndex - 1];
		}

		if (CurrentBarTradeDate != NextBarTradeDate)
		{
			int& r_SufficientData = sc.GetPersistentInt(CurrentBarTradeDate);

			if (r_SufficientData == SufficientData_Unset)
			{
				double RequiredTimespanFraction = Input_MinimumRequiredDataForDayAsPercentage.GetInt() * 0.01;

				if (Array_TimespanForDay[BarIndex]
					>= RequiredTimespanFraction)
				{
					r_SufficientData = SufficientData_True;
				}
				else
				{
					r_SufficientData = SufficientData_False;

					if (Input_EnableDebugLogging.GetYesNo())
					{
		...
		else if (CurrentBarTradeDate != PriorBarTradeDate)// New day. Reset
		{
		...
		if (BarIndex != r_LastIndex)
		{
			double SumOfBarVolume = 0;
			double SumOfCurrentDayCumulativeVolume = 0;
			int NumValues = 0;

			SCDateTime PriorDateTime = BarDateTime;
			const int NumDaysToCalculate = Input_Period.GetInt();
			int DaysBack = 0;

			for (; DaysBack < NumDaysToCalculate; ++DaysBack)
			{
				int PriorIndex = -1;
				int SkipCount = 0;
				while (SkipCount <= 3)
				{
					...
					int& r_SufficientData = sc.GetPersistentInt(PriorTradingDayDate.GetDate());

					if (r_SufficientData == SufficientData_False 
						|| r_SufficientData == SufficientData_Unset)
					{
						SkipCount++;
						continue;
					}
				...
				SumOfBarVolume += sc.BaseData[VolumeSubgraphIndex][PriorIndex];
				SumOfCurrentDayCumulativeVolume += Subgraph_CurrentDayCumulativeVolume[PriorIndex];
			...
			if (NumValues > 0 && DaysBack >= NumDaysToCalculate - 1)
			{
				if (CalculateMedian)
				{
				...
				{
					Array_AverageVolume[BarIndex] = static_cast<float>(SumOfBarVolume / NumValues);

					Array_AverageCumulativeVolumeForPriorDays[BarIndex] = static_cast<float>(SumOfCurrentDayCumulativeVolume / NumValues);
				}

				Subgraph_EarliestReferenceDate[BarIndex] = static_cast<float>(PriorDateTime.GetDate());
			}

			r_LastIndex = BarIndex;
		}
...
SCSFExport scsf_StudySubgraphAsTextAboveBelowBarV2(SCStudyInterfaceRef sc)
{
	...
	if (sc.SetDefaults)
	{
		...
		Input_DisplayAboveOrBelow.Name = "Display Location";
		Input_DisplayAboveOrBelow.SetCustomInputStrings("Top of Bar;Bottom of Bar;Bottom of Chart Region;Top of Chart Region");
		Input_DisplayAboveOrBelow.SetCustomInputIndex(0);
	...
	if (Input_DisplayAboveOrBelow.GetIndex() == 0 || Input_DisplayAboveOrBelow.GetIndex() == 1)
	{
	...
	{	
		...
		if(ValueFromReferencedSubgraph != FLT_MAX && ValueFromReferencedSubgraph != -FLT_MAX)
		{
			...
			else if(DisplayPositionIndex == 2)
			{
				Array_TextPosition[BarIndex] = 5;
			}			
			else if(DisplayPositionIndex == 3)
			{
				Array_TextPosition[BarIndex] = 95;
			}
		}
		else
		{
