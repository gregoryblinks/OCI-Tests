...
/*
    ...
    Studies:
      1. MBO DOM Intelligence - live/custom-replay DOM-style large-order display,
         multi-cycle iceberg/refill confirmation, trade-to-visible-peak and
         absorption ratios, market-response confirmation, and spoof-like filtering.
...
#pragma pack(pop)
    ...
    struct RemovedOrderCandidate
    {
        double DateTime = 0.0;
        uint64_t Quantity = 0;
        uint64_t VisiblePeak = 0;
        uint64_t ExecutedDuringLife = 0;
    };

    // Confirmation state is kept at the price level so a candidate can still be
    // confirmed by a rejection after its source order has disappeared.
    struct IcebergEvidenceState
    {
        bool Active = false;
        bool AlertIssued = false;
        int Kind = ICEBERG_NONE;
        uint64_t SourceOrderID = 0; // Zero for a synthetic replacement chain.
        int RefreshCycles = 0;
        double FirstEvidenceDateTime = 0.0;
        double LastEvidenceDateTime = 0.0;
        double ResponseStartDateTime = 0.0;
        uint64_t VisiblePeak = 0;
        uint64_t AttributedExecuted = 0;
        uint64_t LevelExecutedBaseline = 0;
        uint64_t PeakAggregateQuantity = 0;
        double MaximumCycleTradeToPeakRatio = 0.0;
        bool OverfillObserved = false;
        bool TradeThroughObserved = false;
        int MaximumRejectionTicks = 0;
        double IcebergRatio = 0.0;
    ...
    struct LevelBehavior
    {
        ...
        uint64_t LastAggregateQuantity = 0;
        std::deque<double> SpoofLikeCancellationTimes;
        std::deque<RemovedOrderCandidate> RecentDepletedOrders;
        IcebergEvidenceState Candidate;
        double SuppressUntil = 0.0;
        double IcebergUntil = 0.0;
        int IcebergKind = ICEBERG_NONE;
        int IcebergStrength = 0;
        int ConfirmedRefreshCycles = 0;
        double ConfirmedIcebergRatio = 0.0;
        double ConfirmedAbsorptionRatio = 0.0;
        double ConfirmedMaximumCycleTradeToPeakRatio = 0.0;
        bool ConfirmedOverfillObserved = false;
        bool ConfirmedFailureToTradeThrough = false;
        int ConfirmedRejectionTicks = 0;
        int SpoofScore = 0;
    };
    ...
    struct OrderLife
    {
        ...
        int PersistentRefreshObservations = 0;
    ...
    struct DetectionConfiguration
    {
        ...
        int MinimumRefreshCyclesForAlert = 2;
        double MinimumIcebergRatio = 1.00;
        double MinimumAbsorptionRatio = 1.25;
        double OverfillTradeToVisiblePeakRatio = 1.00;
        bool RequireMarketResponseConfirmation = true;
        int FailureToTradeThroughHoldMilliseconds = 250;
        int MinimumRejectionTicks = 2;
        int MarketResponseWindowMilliseconds = 2500;
    ...
    struct DisplayRow
    {
        ...
        int IcebergKind = ICEBERG_NONE;
        int IcebergStrength = 0;
        int IcebergRefreshCycles = 0;
        double IcebergRatio = 0.0;
        double AbsorptionRatio = 0.0;
        double MaximumCycleTradeToPeakRatio = 0.0;
        bool OverfillObserved = false;
        bool FailureToTradeThrough = false;
        int RejectionTicks = 0;
    ...
    void ResetIcebergEvidence(IcebergEvidenceState& Evidence)
    {
        Evidence = IcebergEvidenceState();
    }

    int IcebergKindPriority(const int Kind)
    {
        if (Kind == ICEBERG_NATIVE_ID)
            return 3;
        if (Kind == ICEBERG_SYNTHETIC)
            return 2;
        if (Kind == ICEBERG_PERSISTENT_REFILL)
            return 1;
        return 0;
    }

    double QuantityRatio(const uint64_t Numerator, const uint64_t Denominator)
    {
    ...
    void PurgeBehaviorQueues(LevelBehavior& Behavior, const double Now, const DetectionConfiguration& Configuration)
    {
        ...
        if (Behavior.IcebergUntil < Now)
        {
            ...
            Behavior.ConfirmedRefreshCycles = 0;
            Behavior.ConfirmedIcebergRatio = 0.0;
            Behavior.ConfirmedAbsorptionRatio = 0.0;
            Behavior.ConfirmedMaximumCycleTradeToPeakRatio = 0.0;
            Behavior.ConfirmedOverfillObserved = false;
            Behavior.ConfirmedFailureToTradeThrough = false;
            Behavior.ConfirmedRejectionTicks = 0;
        }
        if (Behavior.SuppressUntil < Now)
            Behavior.SpoofScore = static_cast<int>(Behavior.SpoofLikeCancellationTimes.size()) * 20;

        if (Behavior.Candidate.Active && Behavior.Candidate.LastEvidenceDateTime != 0.0)
        {
            const int RetentionMilliseconds = std::max(
                Configuration.MarketResponseWindowMilliseconds,
                Configuration.SyntheticReplacementMilliseconds * 4);
            const double IdleMilliseconds = MillisecondsBetween(Now, Behavior.Candidate.LastEvidenceDateTime);
            const bool DisplayExpired = Behavior.IcebergUntil < Now;
            if (IdleMilliseconds > RetentionMilliseconds
                && (!Behavior.Candidate.AlertIssued || DisplayExpired))
            {
                ResetIcebergEvidence(Behavior.Candidate);
            }
        }
    }
    ...
    bool CandidateIsStale(
        const IcebergEvidenceState& Evidence,
        const double Now,
        const DetectionConfiguration& Configuration)
    {
        if (!Evidence.Active || Evidence.LastEvidenceDateTime == 0.0)
            return true;

        const int RetentionMilliseconds = std::max(
            Configuration.MarketResponseWindowMilliseconds,
            Configuration.SyntheticReplacementMilliseconds * 4);
        return MillisecondsBetween(Now, Evidence.LastEvidenceDateTime) > RetentionMilliseconds;
    }

    void StartCandidateResponseWindow(
        IcebergEvidenceState& Evidence,
        const int PreviousRefreshCycles,
        const double Now,
        const DetectionConfiguration& Configuration)
    {
        if (Evidence.RefreshCycles >= Configuration.MinimumRefreshCyclesForAlert
            && Evidence.RefreshCycles > PreviousRefreshCycles)
        {
            // Measure trade-through and rejection after the latest qualifying
            // refill/replacement, not from an older interaction at the level.
            Evidence.ResponseStartDateTime = Now;
            Evidence.TradeThroughObserved = false;
            Evidence.MaximumRejectionTicks = 0;
        }
    }

    void RecordOrderIcebergEvidence(
        LevelBehavior& Behavior,
        const OrderKey& Key,
        const int Kind,
        const int RefreshCycles,
        const uint64_t CumulativeExecutedAtOrder,
        const uint64_t VisiblePeak,
        const double CycleTradeToPeakRatio,
        const uint64_t TotalExecutedBeforeCycle,
        const uint64_t CurrentAggregateQuantity,
        const double Now,
        const DetectionConfiguration& Configuration)
    {
        IcebergEvidenceState& Evidence = Behavior.Candidate;
        const bool SameSource = Evidence.Active
            && Evidence.Kind != ICEBERG_SYNTHETIC
            && Evidence.SourceOrderID == Key.OrderID;
        const bool Stale = CandidateIsStale(Evidence, Now, Configuration);

        if (!SameSource)
        {
            const bool ExistingProtected = Evidence.Active && !Stale
                && ((Evidence.AlertIssued && IcebergKindPriority(Evidence.Kind) >= IcebergKindPriority(Kind))
                    || (!Evidence.AlertIssued
                        && Evidence.RefreshCycles >= RefreshCycles
                        && IcebergKindPriority(Evidence.Kind) >= IcebergKindPriority(Kind)));
            if (ExistingProtected)
                return;

            ResetIcebergEvidence(Evidence);
            Evidence.Active = true;
            Evidence.Kind = Kind;
            Evidence.SourceOrderID = Key.OrderID;
            Evidence.FirstEvidenceDateTime = Now;
            Evidence.LevelExecutedBaseline = TotalExecutedBeforeCycle;
            Evidence.PeakAggregateQuantity = std::max(
                std::max(CurrentAggregateQuantity, Behavior.LastAggregateQuantity),
                VisiblePeak);
        }

        const int PreviousRefreshCycles = Evidence.RefreshCycles;
        const int PreviousKind = Evidence.Kind;
        if (IcebergKindPriority(Kind) > IcebergKindPriority(Evidence.Kind))
            Evidence.Kind = Kind;
        Evidence.SourceOrderID = Key.OrderID;
        Evidence.RefreshCycles = std::max(Evidence.RefreshCycles, RefreshCycles);
        Evidence.LastEvidenceDateTime = Now;
        Evidence.VisiblePeak = std::max(Evidence.VisiblePeak, VisiblePeak);
        Evidence.AttributedExecuted = std::max(Evidence.AttributedExecuted, CumulativeExecutedAtOrder);
        Evidence.PeakAggregateQuantity = std::max(
            Evidence.PeakAggregateQuantity,
            std::max(CurrentAggregateQuantity, Behavior.LastAggregateQuantity));

        Evidence.MaximumCycleTradeToPeakRatio = std::max(
            Evidence.MaximumCycleTradeToPeakRatio,
            CycleTradeToPeakRatio);
        if (CycleTradeToPeakRatio >= Configuration.OverfillTradeToVisiblePeakRatio)
            Evidence.OverfillObserved = true;

        const bool KindUpgraded = IcebergKindPriority(Evidence.Kind) > IcebergKindPriority(PreviousKind);
        if (KindUpgraded && Evidence.AlertIssued)
            Evidence.AlertIssued = false;

        StartCandidateResponseWindow(Evidence, PreviousRefreshCycles, Now, Configuration);
        if (KindUpgraded && Evidence.RefreshCycles >= Configuration.MinimumRefreshCyclesForAlert)
        {
            Evidence.ResponseStartDateTime = Now;
            Evidence.TradeThroughObserved = false;
            Evidence.MaximumRejectionTicks = 0;
    ...
    void RecordSyntheticIcebergEvidence(
        LevelBehavior& Behavior,
        const RemovedOrderCandidate& Removed,
        const BookOrder& Replacement,
        const uint64_t CurrentAggregateQuantity,
        const double Now,
        const DetectionConfiguration& Configuration)
    {
        IcebergEvidenceState& Evidence = Behavior.Candidate;
        const int ChainWindowMilliseconds = std::max(
            Configuration.MarketResponseWindowMilliseconds,
            Configuration.SyntheticReplacementMilliseconds * 4);
        const bool ContinueChain = Evidence.Active
            && Evidence.Kind == ICEBERG_SYNTHETIC
            && Evidence.SourceOrderID == 0
            && Evidence.LastEvidenceDateTime != 0.0
            && MillisecondsBetween(Now, Evidence.LastEvidenceDateTime) <= ChainWindowMilliseconds;

        if (!ContinueChain)
        {
            const bool Stale = CandidateIsStale(Evidence, Now, Configuration);
            const bool ExistingProtected = Evidence.Active && !Stale
                && (Evidence.AlertIssued || IcebergKindPriority(Evidence.Kind) > IcebergKindPriority(ICEBERG_SYNTHETIC));
            if (ExistingProtected)
                return;
            ...
            Evidence.SourceOrderID = 0;
            Evidence.FirstEvidenceDateTime = Now;
            Evidence.LevelExecutedBaseline = Behavior.TotalExecuted >= Removed.ExecutedDuringLife
                ? Behavior.TotalExecuted - Removed.ExecutedDuringLife
                : 0;
            Evidence.PeakAggregateQuantity = std::max(
                std::max(CurrentAggregateQuantity, Behavior.LastAggregateQuantity),
                std::max(Removed.VisiblePeak, Replacement.Quantity));
        }

        const int PreviousRefreshCycles = Evidence.RefreshCycles;
        ++Evidence.RefreshCycles;
        Evidence.LastEvidenceDateTime = Now;
        Evidence.VisiblePeak = std::max(
            Evidence.VisiblePeak,
            std::max(Removed.VisiblePeak, Replacement.Quantity));
        Evidence.AttributedExecuted += Removed.ExecutedDuringLife;
        Evidence.PeakAggregateQuantity = std::max(
            Evidence.PeakAggregateQuantity,
            std::max(CurrentAggregateQuantity, Behavior.LastAggregateQuantity));

        const uint64_t RemovedVisiblePeak = Removed.VisiblePeak == 0 ? Removed.Quantity : Removed.VisiblePeak;
        const double CycleTradeToPeakRatio = QuantityRatio(Removed.ExecutedDuringLife, RemovedVisiblePeak);
        Evidence.MaximumCycleTradeToPeakRatio = std::max(
            Evidence.MaximumCycleTradeToPeakRatio,
            CycleTradeToPeakRatio);
        if (CycleTradeToPeakRatio >= Configuration.OverfillTradeToVisiblePeakRatio)
    ...
    void UpdateAndEvaluateIcebergCandidates(
        DetectionEngine& Engine,
        const BookFrame& Frame,
        const std::map<LevelKey, uint64_t>& CurrentAggregateQuantities,
        const DetectionConfiguration& Configuration,
        std::vector<DetectionEvent>& Events)
    {
        const double Now = Frame.DateTime;
        const double OneMillisecond = 0.001 / SECONDS_PER_DAY;

        for (auto& Pair : Engine.Levels)
        {
            const LevelKey& Level = Pair.first;
            LevelBehavior& Behavior = Pair.second;
            IcebergEvidenceState& Evidence = Behavior.Candidate;
            if (!Evidence.Active)
                continue;

            const auto AggregateIterator = CurrentAggregateQuantities.find(Level);
            if (AggregateIterator != CurrentAggregateQuantities.end())
            {
                Evidence.PeakAggregateQuantity = std::max(
                    Evidence.PeakAggregateQuantity,
                    AggregateIterator->second);
            }

            if (Evidence.ResponseStartDateTime != 0.0)
            {
                for (const TradeEvent& Trade : Frame.Trades)
                {
                    if (Trade.DateTime + OneMillisecond < Evidence.ResponseStartDateTime)
                        continue;

                    if (Trade.Side == Level.Side)
                    {
                        if ((Level.Side == SIDE_ASK && Trade.PriceInTicks > Level.PriceInTicks)
                            || (Level.Side == SIDE_BID && Trade.PriceInTicks < Level.PriceInTicks))
                        {
                            Evidence.TradeThroughObserved = true;
                        }
                    }

                    const int RejectionTicks = Level.Side == SIDE_ASK
                        ? std::max(0, Level.PriceInTicks - Trade.PriceInTicks)
                        : std::max(0, Trade.PriceInTicks - Level.PriceInTicks);
                    Evidence.MaximumRejectionTicks = std::max(
                        Evidence.MaximumRejectionTicks,
                        RejectionTicks);
                }

                // A quote moving beyond the defended level also invalidates a
                // no-trade-through confirmation, even if the corresponding trade
                // print was not present in this snapshot interval.
                if ((Level.Side == SIDE_ASK
                        && Frame.BestAskInTicks != 0
                        && Frame.BestAskInTicks > Level.PriceInTicks)
                    || (Level.Side == SIDE_BID
                        && Frame.BestBidInTicks != 0
                        && Frame.BestBidInTicks < Level.PriceInTicks))
                {
                    Evidence.TradeThroughObserved = true;
                }

                const int QuoteRejectionTicks = Level.Side == SIDE_ASK
                    ? (Frame.BestAskInTicks == 0 ? 0 : std::max(0, Level.PriceInTicks - Frame.BestAskInTicks))
                    : (Frame.BestBidInTicks == 0 ? 0 : std::max(0, Frame.BestBidInTicks - Level.PriceInTicks));
                Evidence.MaximumRejectionTicks = std::max(
                    Evidence.MaximumRejectionTicks,
                    QuoteRejectionTicks);
            }

            Evidence.IcebergRatio = QuantityRatio(Evidence.AttributedExecuted, Evidence.VisiblePeak);
            const uint64_t ExecutedSinceCandidate = Behavior.TotalExecuted >= Evidence.LevelExecutedBaseline
                ? Behavior.TotalExecuted - Evidence.LevelExecutedBaseline
                : 0;
            Evidence.AbsorptionRatio = QuantityRatio(
                ExecutedSinceCandidate,
                Evidence.PeakAggregateQuantity);

            if (Evidence.AlertIssued
                || Evidence.RefreshCycles < Configuration.MinimumRefreshCyclesForAlert
                || Evidence.ResponseStartDateTime == 0.0)
            {
                continue;
            }

            const bool RatioConfirmed = Evidence.OverfillObserved
                || Evidence.IcebergRatio >= Configuration.MinimumIcebergRatio
                || Evidence.AbsorptionRatio >= Configuration.MinimumAbsorptionRatio;
            if (!RatioConfirmed)
                continue;

            const double ResponseAgeMilliseconds = MillisecondsBetween(Now, Evidence.ResponseStartDateTime);
            const bool FailureToTradeThroughConfirmed = !Evidence.TradeThroughObserved
                && ResponseAgeMilliseconds >= Configuration.FailureToTradeThroughHoldMilliseconds;
            const bool RejectionConfirmed = Evidence.MaximumRejectionTicks >= Configuration.MinimumRejectionTicks;
            const bool MarketResponseConfirmed = !Configuration.RequireMarketResponseConfirmation
                || FailureToTradeThroughConfirmed
                || RejectionConfirmed;
            if (!MarketResponseConfirmed)
                continue;

            int Strength = Evidence.Kind == ICEBERG_NATIVE_ID ? 3 : 2;
            if (Evidence.RefreshCycles > Configuration.MinimumRefreshCyclesForAlert)
                ++Strength;
            if (Evidence.OverfillObserved)
                ++Strength;
            if (RejectionConfirmed
                || std::max(Evidence.IcebergRatio, Evidence.AbsorptionRatio) >= 2.0)
            {
                ++Strength;
            }
            Strength = Clamp(Strength, 2, 5);

            MarkIceberg(Behavior, Evidence.Kind, Strength, Now, Configuration);
            Behavior.ConfirmedRefreshCycles = Evidence.RefreshCycles;
            Behavior.ConfirmedIcebergRatio = Evidence.IcebergRatio;
            Behavior.ConfirmedAbsorptionRatio = Evidence.AbsorptionRatio;
            Behavior.ConfirmedMaximumCycleTradeToPeakRatio = Evidence.MaximumCycleTradeToPeakRatio;
            Behavior.ConfirmedOverfillObserved = Evidence.OverfillObserved;
            Behavior.ConfirmedFailureToTradeThrough = FailureToTradeThroughConfirmed;
            Behavior.ConfirmedRejectionTicks = Evidence.MaximumRejectionTicks;
            Evidence.AlertIssued = true;

            const char* DetectionName = Evidence.Kind == ICEBERG_NATIVE_ID
                ? "CONFIRMED SAME-ID ICEBERG"
                : (Evidence.Kind == ICEBERG_SYNTHETIC
                    ? "CONFIRMED SYNTHETIC ICEBERG"
                    : "CONFIRMED PERSISTENT REFILL");
            const char* ResponseName = RejectionConfirmed ? "REJECTION" : "NO TRADE-THROUGH";
            const int ResponseValue = RejectionConfirmed
                ? Evidence.MaximumRejectionTicks
                : static_cast<int>(std::floor(ResponseAgeMilliseconds + 0.5));
            const char* ResponseUnit = RejectionConfirmed ? "ticks" : "ms";

            DetectionEvent Event;
            Event.Type = 1;
            Event.Level = Level;
            Event.IcebergKind = Evidence.Kind;
            Event.Strength = Strength;
            Event.Message.Format(
                "%s %s at price ticks %d | cycles %d | order trade/visible peak %.2fx | level absorption %.2fx | max cycle/peak %.2fx | overfill %s | response %s %d %s",
                DetectionName,
                Level.Side == SIDE_BID ? "BID" : "ASK",
                Level.PriceInTicks,
                Evidence.RefreshCycles,
                Evidence.IcebergRatio,
                Evidence.AbsorptionRatio,
                Evidence.MaximumCycleTradeToPeakRatio,
                Evidence.OverfillObserved ? "YES" : "NO",
                ResponseName,
                ResponseValue,
                ResponseUnit);
        ...
        std::vector<DetectionEvent>& Events)
    {
        ...
        std::map<LevelKey, uint64_t> CurrentAggregateQuantities;
        for (const BookLevel& Level : Frame.Levels)
        {
            CurrentAggregateQuantities[Level.Key] = Level.AggregateQuantity;
            uint64_t VisibleQuantityAhead = 0;
            for (const BookOrder& Order : Level.Orders)
            {
        ...
        for (auto Iterator = Engine.ActiveOrders.begin(); Iterator != Engine.ActiveOrders.end();)
        {
            ...
            const uint64_t ExecutedSinceLastAtPrice = Behavior.TotalExecuted >= Life.LastExecutedCounter
                ...
                : 0;
            const uint64_t FinalEstimatedExecution = EstimateExecutionAtTrackedOrder(
                ExecutedSinceLastAtPrice,
                Life.LastVisibleQuantityAhead);
            const uint64_t ExecutedDuringLife = Life.EstimatedExecutedAtOrder + FinalEstimatedExecution;
            const double LifeMilliseconds = MillisecondsBetween(Now, Life.FirstSeen);
            const double ExecutedFraction = QuantityRatio(ExecutedDuringLife, Life.MaximumQuantity);

            // Preserve a qualified order-level candidate after the visible order
            // disappears so a later rejection can still confirm the signal.
            if (Behavior.Candidate.Active
                && Behavior.Candidate.Kind != ICEBERG_SYNTHETIC
                && Behavior.Candidate.SourceOrderID == Key.OrderID)
            {
                Behavior.Candidate.AttributedExecuted = std::max(
                    Behavior.Candidate.AttributedExecuted,
                    ExecutedDuringLife);
                Behavior.Candidate.VisiblePeak = std::max(
                    Behavior.Candidate.VisiblePeak,
                    Life.MaximumQuantity);
                const double FinalCycleTradeToPeakRatio = QuantityRatio(
                    FinalEstimatedExecution,
                    Life.MaximumQuantity);
                Behavior.Candidate.MaximumCycleTradeToPeakRatio = std::max(
                    Behavior.Candidate.MaximumCycleTradeToPeakRatio,
                    FinalCycleTradeToPeakRatio);
                if (FinalCycleTradeToPeakRatio >= Configuration.OverfillTradeToVisiblePeakRatio)
                    Behavior.Candidate.OverfillObserved = true;
            }
            ...
            else if (ExecutedDuringLife >= Configuration.MinimumIcebergExecution)
            {
                ...
                Candidate.Quantity = Life.MaximumQuantity;
                Candidate.VisiblePeak = Life.MaximumQuantity;
        ...
        for (const auto& Pair : CurrentOrders)
        {
            ...
            const LevelKey LevelKeyValue{Key.Side, Key.PriceInTicks};
            LevelBehavior& Behavior = Engine.Levels[LevelKeyValue];
            const auto AggregateIterator = CurrentAggregateQuantities.find(LevelKeyValue);
            const uint64_t CurrentAggregateQuantity = AggregateIterator == CurrentAggregateQuantities.end()
                ? 0
                : AggregateIterator->second;
            ...
            if (LifeIterator == Engine.ActiveOrders.end())
            {
                ...
                {
                    for (auto CandidateIterator = Behavior.RecentDepletedOrders.begin();
                        CandidateIterator != Behavior.RecentDepletedOrders.end();
                        ++CandidateIterator)
                    {
                        const double DelayMilliseconds = MillisecondsBetween(Now, CandidateIterator->DateTime);
                        const uint64_t CandidateSize = CandidateIterator->VisiblePeak == 0
                            ? CandidateIterator->Quantity
                            : CandidateIterator->VisiblePeak;
                        const uint64_t Larger = std::max(CandidateSize, Current.Quantity);
                        const uint64_t Difference = Larger - std::min(CandidateSize, Current.Quantity);
                        const double SizeDifferenceFraction = Larger == 0
                            ? 1.0
                            : static_cast<double>(Difference) / static_cast<double>(Larger);
                        ...
                        {
                            RecordSyntheticIcebergEvidence(
                                Behavior,
                                *CandidateIterator,
                                Current,
                                CurrentAggregateQuantity,
                                Now,
                                Configuration);
            ...
            const uint64_t QuantityIncrease = Current.Quantity > Life.LastQuantity
                ...
                : 0;
            const uint64_t PriorVisiblePeak = std::max(Life.MaximumQuantity, Life.LastQuantity);
            const uint64_t VisiblePeakIncludingCurrent = std::max(PriorVisiblePeak, Current.Quantity);
            const uint64_t CumulativeEstimatedExecution = Life.EstimatedExecutedAtOrder + EstimatedExecutedAtOrder;
            const double CycleTradeToPeakRatio = QuantityRatio(
                EstimatedExecutedAtOrder,
                PriorVisiblePeak);
            const bool OverfillThisCycle = CycleTradeToPeakRatio >= Configuration.OverfillTradeToVisiblePeakRatio;

            bool RefreshEvidenceObserved = false;
                ...
                && QuantityIncrease >= Configuration.MinimumNativeRefreshQuantity)
            {
                ...
                && static_cast<double>(Current.Quantity)
                    >= static_cast<double>(Life.LastQuantity) * Configuration.PersistentQuantityMinimumFraction)
            {
                ++Life.PersistentRefreshObservations;
                RefreshEvidenceObserved = true;
                TriggeredKind = Life.RefreshObservations > 0
                    ? ICEBERG_NATIVE_ID
                    : ICEBERG_PERSISTENT_REFILL;
            }

            const int TotalRefreshCycles = Life.RefreshObservations + Life.PersistentRefreshObservations;
            if (RefreshEvidenceObserved)
            {
                const uint64_t TotalExecutedBeforeCycle = Behavior.TotalExecuted >= ExecutedSinceLastAtPrice
                    ? Behavior.TotalExecuted - ExecutedSinceLastAtPrice
                    : 0;
                RecordOrderIcebergEvidence(
                    Behavior,
                    Key,
                    TriggeredKind,
                    TotalRefreshCycles,
                    CumulativeEstimatedExecution,
                    VisiblePeakIncludingCurrent,
                    CycleTradeToPeakRatio,
                    TotalExecutedBeforeCycle,
                    CurrentAggregateQuantity,
                    Now,
                    Configuration);
            }
            else if (Behavior.Candidate.Active
                && Behavior.Candidate.Kind != ICEBERG_SYNTHETIC
                && Behavior.Candidate.SourceOrderID == Key.OrderID)
            {
                // Continue updating formal ratios between explicit refresh cycles.
                Behavior.Candidate.AttributedExecuted = std::max(
                    Behavior.Candidate.AttributedExecuted,
                    CumulativeEstimatedExecution);
                Behavior.Candidate.VisiblePeak = std::max(
                    Behavior.Candidate.VisiblePeak,
                    VisiblePeakIncludingCurrent);
                Behavior.Candidate.PeakAggregateQuantity = std::max(
                    Behavior.Candidate.PeakAggregateQuantity,
                    CurrentAggregateQuantity);
                Behavior.Candidate.MaximumCycleTradeToPeakRatio = std::max(
                    Behavior.Candidate.MaximumCycleTradeToPeakRatio,
                    CycleTradeToPeakRatio);
                if (OverfillThisCycle)
                    Behavior.Candidate.OverfillObserved = true;
            }

            Life.EstimatedExecutedAtOrder = CumulativeEstimatedExecution;
            Life.LastSeen = Now;
            Life.LastQuantity = Current.Quantity;
            Life.MaximumQuantity = VisiblePeakIncludingCurrent;
            Life.LastExecutedCounter = Behavior.TotalExecuted;
            Life.LastVisibleQuantityAhead = Observation.VisibleQuantityAhead;
            Life.MinimumDistanceToTouchTicks = std::min(
                Life.MinimumDistanceToTouchTicks,
                DistanceToTouch(Key, Frame));
        }

        // Update aggregate peaks before evaluating absorption and market response.
        for (const auto& Pair : CurrentAggregateQuantities)
            Engine.Levels[Pair.first].LastAggregateQuantity = Pair.second;

        UpdateAndEvaluateIcebergCandidates(
            Engine,
            Frame,
            CurrentAggregateQuantities,
            Configuration,
            Events);
        ...
        std::vector<DisplayRow>& Rows)
    {
        ...
        for (const BookLevel& Level : Frame.Levels)
        {
            ...
            if (BehaviorIterator != Engine.Levels.end())
            {
                ...
                {
                    Row.IcebergRefreshCycles = Behavior.ConfirmedRefreshCycles;
                    Row.IcebergRatio = Behavior.ConfirmedIcebergRatio;
                    Row.AbsorptionRatio = Behavior.ConfirmedAbsorptionRatio;
                    Row.MaximumCycleTradeToPeakRatio = Behavior.ConfirmedMaximumCycleTradeToPeakRatio;
                    Row.OverfillObserved = Behavior.ConfirmedOverfillObserved;
                    Row.FailureToTradeThrough = Behavior.ConfirmedFailureToTradeThrough;
                    Row.RejectionTicks = Behavior.ConfirmedRejectionTicks;
                }
    ...
    uint64_t ComputeDisplayHash(const std::vector<DisplayRow>& Rows, const VisualConfiguration& Visual)
    {
        ...
        HashMix(Hash, static_cast<uint64_t>(Visual.ShowIcebergMetrics));
        ...
        for (const DisplayRow& Row : Rows)
        {
            ...
            HashMix(Hash, static_cast<uint64_t>(Row.IcebergRefreshCycles));
            HashMix(Hash, static_cast<uint64_t>(std::floor(Row.IcebergRatio * 100.0 + 0.5)));
            HashMix(Hash, static_cast<uint64_t>(std::floor(Row.AbsorptionRatio * 100.0 + 0.5)));
            HashMix(Hash, static_cast<uint64_t>(std::floor(Row.MaximumCycleTradeToPeakRatio * 100.0 + 0.5)));
            HashMix(Hash, static_cast<uint64_t>(Row.OverfillObserved));
            HashMix(Hash, static_cast<uint64_t>(Row.FailureToTradeThrough));
            HashMix(Hash, static_cast<uint64_t>(Row.RejectionTicks));
    ...
    std::string FormatRowText(const DisplayRow& Row, const VisualConfiguration& Visual)
    {
        ...
        if (Row.IcebergActive)
        {
            ...
            if (Visual.ShowIcebergMetrics)
            {
                const double DisplayRatio = std::max(Row.IcebergRatio, Row.AbsorptionRatio);
                const int RatioTenths = std::max(0, static_cast<int>(std::floor(DisplayRatio * 10.0 + 0.5)));
                Text << " C" << Row.IcebergRefreshCycles
                    << " x" << RatioTenths / 10 << '.' << RatioTenths % 10;
                if (Row.OverfillObserved)
                    Text << " O";
                if (Row.RejectionTicks > 0)
                    Text << " R" << Row.RejectionTicks;
                else if (Row.FailureToTradeThrough)
                    Text << " H";
            }
            Text << ' ';
        }
...
SCSFExport scsf_MBODOMIntelligence(SCStudyInterfaceRef sc)
{
    ...
    SCInputRef MinimumRefreshCyclesForAlert = sc.Input[50];
    SCInputRef MinimumIcebergRatioPercent = sc.Input[51];
    SCInputRef MinimumAbsorptionRatioPercent = sc.Input[52];
    SCInputRef OverfillTradeToVisiblePeakPercent = sc.Input[53];
    SCInputRef RequireMarketResponseConfirmation = sc.Input[54];
    SCInputRef FailureToTradeThroughHoldMilliseconds = sc.Input[55];
    SCInputRef MinimumRejectionTicks = sc.Input[56];
    SCInputRef MarketResponseWindowMilliseconds = sc.Input[57];
    SCInputRef ShowIcebergMetricsInDOM = sc.Input[58];

    if (sc.SetDefaults)
    {
        sc.GraphName = "MBO DOM Intelligence - Confirmed Multi-Cycle Iceberg, Absorption, Spoof-Like Filter, Recorder/Replay";
        sc.StudyDescription = "Displays large aggregate bid/ask depth with individual MBO detail, confirms iceberg/refill candidates only after configurable multi-cycle evidence, applies explicit trade-to-visible-peak and level absorption ratios, requires either failure to trade through or subsequent rejection by default, filters spoof-like liquidity heuristically, and records/reads a custom MBO snapshot file for replay. Public order-book data cannot establish spoofing intent or guarantee complete iceberg detection.";
        sc.AutoLoop = 0;
        sc.GraphRegion = 0;
        sc.UpdateAlways = 1;
        ...
        MinimumRefreshCyclesForAlert.Name = "Iceberg Confirmation: Minimum Refresh / Replacement Cycles";
        MinimumRefreshCyclesForAlert.SetInt(2);
        MinimumRefreshCyclesForAlert.SetIntLimits(2, 10);

        MinimumIcebergRatioPercent.Name = "Iceberg Confirmation: Minimum Attributed Trade / Visible Peak %";
        MinimumIcebergRatioPercent.SetFloat(100.0f);
        MinimumIcebergRatioPercent.SetFloatLimits(10.0f, 1000.0f);

        MinimumAbsorptionRatioPercent.Name = "Iceberg Confirmation: Minimum Level Execution / Peak Aggregate %";
        MinimumAbsorptionRatioPercent.SetFloat(125.0f);
        MinimumAbsorptionRatioPercent.SetFloatLimits(10.0f, 1000.0f);

        OverfillTradeToVisiblePeakPercent.Name = "Iceberg Confirmation: Single-Cycle Overfill Trade / Visible Peak %";
        OverfillTradeToVisiblePeakPercent.SetFloat(100.0f);
        OverfillTradeToVisiblePeakPercent.SetFloatLimits(25.0f, 1000.0f);

        RequireMarketResponseConfirmation.Name = "Iceberg Confirmation: Require No-Trade-Through Or Rejection";
        RequireMarketResponseConfirmation.SetYesNo(1);

        FailureToTradeThroughHoldMilliseconds.Name = "Iceberg Confirmation: No-Trade-Through Hold (ms)";
        FailureToTradeThroughHoldMilliseconds.SetInt(250);
        FailureToTradeThroughHoldMilliseconds.SetIntLimits(0, 10000);

        MinimumRejectionTicks.Name = "Iceberg Confirmation: Minimum Subsequent Rejection (ticks)";
        MinimumRejectionTicks.SetInt(2);
        MinimumRejectionTicks.SetIntLimits(1, 100);

        MarketResponseWindowMilliseconds.Name = "Iceberg Confirmation: Candidate / Response Window (ms)";
        MarketResponseWindowMilliseconds.SetInt(2500);
        MarketResponseWindowMilliseconds.SetIntLimits(250, 30000);

        ShowIcebergMetricsInDOM.Name = "DOM Text: Show Iceberg Cycles / Ratio / Response Metrics";
        ShowIcebergMetricsInDOM.SetYesNo(1);
    ...
    Detection.MinimumRefreshCyclesForAlert = Clamp(MinimumRefreshCyclesForAlert.GetInt(), 2, 10);
    Detection.MinimumIcebergRatio = Clamp(MinimumIcebergRatioPercent.GetFloat() / 100.0, 0.10, 10.0);
    Detection.MinimumAbsorptionRatio = Clamp(MinimumAbsorptionRatioPercent.GetFloat() / 100.0, 0.10, 10.0);
    Detection.OverfillTradeToVisiblePeakRatio = Clamp(OverfillTradeToVisiblePeakPercent.GetFloat() / 100.0, 0.25, 10.0);
    Detection.RequireMarketResponseConfirmation = RequireMarketResponseConfirmation.GetYesNo() != 0;
    Detection.FailureToTradeThroughHoldMilliseconds = Clamp(FailureToTradeThroughHoldMilliseconds.GetInt(), 0, 10000);
    Detection.MinimumRejectionTicks = Clamp(MinimumRejectionTicks.GetInt(), 1, 100);
    Detection.MarketResponseWindowMilliseconds = std::max(
        Detection.FailureToTradeThroughHoldMilliseconds,
        Clamp(MarketResponseWindowMilliseconds.GetInt(), 250, 30000));
    ...
    State->Visual.ShowIcebergMetrics = ShowIcebergMetricsInDOM.GetYesNo() != 0;
