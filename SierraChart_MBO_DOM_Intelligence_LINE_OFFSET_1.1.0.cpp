...
/*
    ...
    Revision: 1.1.0 - chart-line candle clearance
...
#pragma pack(pop)
    ...
    struct VisualConfiguration
    {
        ...
        int LineStartOffsetBars = 2;
        bool DrawLinesUnderCandles = true;
    };
    ...
    uint64_t ComputeDisplayHash(const std::vector<DisplayRow>& Rows, const VisualConfiguration& Visual)
    {
        ...
        HashMix(Hash, static_cast<uint64_t>(Visual.LineStartOffsetBars));
        HashMix(Hash, static_cast<uint64_t>(Visual.DrawLinesUnderCandles));
    ...
    {
        uint64_t DisplayHash = ComputeDisplayHash(State.DisplayRows, State.Visual);
        // The start point is relative to the newest candle, so force an
        // adjustment whenever a new chart bar is added even if DOM values did
        // not change during that exact study call.
        HashMix(DisplayHash, static_cast<uint64_t>(sc.ArraySize));
        ...
        if (State.Visual.DrawLargeLevelLines && sc.ArraySize > 0 && sc.TickSize > 0.0f)
        {
            for (const DisplayRow& Row : State.DisplayRows)
            {
                ...
                Tool.Region = 0;
                Tool.BeginIndex = sc.ArraySize - 1 + std::max(1, State.Visual.LineStartOffsetBars);
                Tool.BeginValue = static_cast<double>(Row.Level.PriceInTicks) * sc.TickSize;
                Tool.Color = RowColor(Row, State.Visual);
                Tool.DrawUnderneathMainGraph = State.Visual.DrawLinesUnderCandles ? 1 : 0;
...
SCSFExport scsf_MBODOMIntelligence(SCStudyInterfaceRef sc)
{
    ...
    SCInputRef LineStartOffsetBars = sc.Input[50];
    SCInputRef DrawLinesUnderCandles = sc.Input[51];
    ...
    if (sc.SetDefaults)
    {
        ...
        LineStartOffsetBars.Name = "Chart: Line Start Offset Bars To Right Of Last Candle";
        LineStartOffsetBars.SetInt(2);
        LineStartOffsetBars.SetIntLimits(1, 50);

        DrawLinesUnderCandles.Name = "Chart: Draw Horizontal Lines Under Candles";
        DrawLinesUnderCandles.SetYesNo(1);
    ...
    State->Visual.LineStartOffsetBars = Clamp(LineStartOffsetBars.GetInt(), 1, 50);
    State->Visual.DrawLinesUnderCandles = DrawLinesUnderCandles.GetYesNo() != 0;
